document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateMapCoverage() {
        const allowedResolutions = new Set(resState.map((on, i) => on ? i : -1).filter(v => v !== -1));
        fetch('coverage.json')
        .then(r => r.ok ? r.json() : [])
        .then(data => {
            coverageLayer.clearLayers();
            data.forEach(tile => {
                if (!allowedResolutions.has(tile.sizeId)) return; // Applica il filtro

                // La chiamata onclick ora punta alla nuova funzione globale
                const popupHtml = `
                ID: ${tile.id}<br>
                Risoluzione: ${tile.sizeId}<br>
                <button onclick="showTileInPanel(${tile.id})">Visualizza Anteprima</button>
                `;
                const bounds = [[tile.bbox.latLL, tile.bbox.lonLL], [tile.bbox.latUR, tile.bbox.lonUR]];
                L.rectangle(bounds, { ...getStyleForSizeId(tile.sizeId), fillOpacity: currentOpacity, opacity: 1 })
                .addTo(coverageLayer)
                .bindPopup(popupHtml);
            });
    ...
    function updateMapCoverage() {
        ...
        .then(data => {
            coverageLayer.clearLayers();
            data.forEach(tile => {
                // La chiamata onclick ora punta alla nostra nuova funzione globale
                const popupHtml = `
                ID: ${tile.id}<br>
                Risoluzione: ${tile.sizeId}<br>
                <button onclick="showTileInPanel(${tile.id})">Visualizza Anteprima</button>
    ...
    function updateAircraftPosition(data) {
        ...
        if (!aircraftMarker) {
            ...
            aircraftMarker = L.marker(latLng, { icon: icon })
            .addTo(map)
            .bindTooltip(tooltipContent);

            // *** attach rotation capability ***
            aircraftMarker.setRotationAngle = function (angle) {
    ...
    function showTileInPanel(id) {
        console.log(`Richiesta anteprima per il tile ${id}`);

        // Riferimenti agli elementi del pannello di preview
        const imgPreview = document.getElementById('tilePreview');
        const btnDownload = document.getElementById('downloadBtn');

        // Costruisci l'URL per l'immagine a 512px
        const imageUrl = `/preview?id=${id}&w=512`;

        // 1. Imposta la sorgente dell'immagine. Il browser la scaricherà.
        imgPreview.src = imageUrl;

        // 2. Rendi visibili l'immagine e il bottone di download
        imgPreview.style.display = 'block';
        btnDownload.style.display = 'block';

        // 3. Configura il bottone di download
        btnDownload.onclick = () => {
            const a = document.createElement('a');
            a.href = imageUrl;
            a.download = `${id}_preview.png`;
        ...
        .then(data => {
            ...
            data.forEach(tile => {
                ...
                .bindPopup(
                    `ID: ${tile.id}<br>
                    Risoluzione: ${tile.sizeId}<br>
                    <button onclick="openTilePng(${tile.id})">Visualizza PNG</button>`
                )
                .addTo(coverageLayer);
            });
    ...
    function showTileInPanel(id) {
        const img   = document.getElementById('tilePreview');
        const btn   = document.getElementById('downloadBtn');
        const src   = `/preview?id=${id}&w=1024`;
    ...
    map.on('click', (e) => {
        const tileId = pickTileId(e); // your existing helper
        if (tileId) showTileInPanel(tileId);
    });
    ...
    window.openTilePng = function (id) {
        console.log('Clicked tile', id);          // ← must appear
        const url = `/preview?id=${id}&w=1024`;
        console.log('URL', url);                  // ← must appear
        window.open(url, '_blank');               // ← simplest test
    };

    async function pollFGFS() {
        try {
            const res = await fetch('/api/fgfs-status');
            const data = await res.json();   // <-- data is now defined
            updateAircraftPosition(data);
        } catch (e) {
            console.warn('FGFS polling error', e);
        }
    }
    setInterval(pollFGFS, 1000);
